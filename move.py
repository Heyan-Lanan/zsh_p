...
def loss_2(q):
    ...
    loss4 = 1.0 / np.sum((new_pose[:3, 3] - finger_12) ** 2)
    loss5 = 1.0 / np.sum((new_pose_j6[:3, 3] - finger_12) ** 2)
    # print(loss1, loss2, loss3)
    return loss1 + loss2 + loss3 + loss4 + loss5


def pick(base_cyclic, tolerance=0.01, speed_mu_1=0.5):
    global wrist, target_id, target_pose, finger_12
    # print('Move to: ', target_pose)
    while True:
        # t1 = time.time()
        while not rospy.is_shutdown():
            if rospy.has_param('wrist') and rospy.has_param('finger_12'):
                wrist = rospy.get_param('wrist')
                finger_12 = rospy.get_param('finger_12')
                break
        ...
        tool_dis = min(np.sum((cur_pose - wrist) ** 2) ** 0.5, np.sum((cur_pose - finger_12) ** 2) ** 0.5)
        loss = loss_1
        speed_mu = speed_mu_1
        print(tool_dis)
        if tool_dis < 0.2:
            ...
            speed_mu = 0.4
            while not rospy.is_shutdown():
                if rospy.has_param('/chess_available'):
                    chess_available_1 = np.array(rospy.get_param("/chess_available"))
                    target_id = np.min(chess_available_1)
                    # print(target_id)
                    break
            param_name = '/chess_' + str(target_id)
            # print(target_id)
            while not rospy.is_shutdown():
                if rospy.has_param(param_name):
                    target_pose = rospy.get_param(param_name)
                    target_pose[2] = 0.036
                    break
...
def place(base_cyclic, tolerance=0.01, speed_mu_1=0.5):
    global wrist, finger_12
    # print('Move to: ', target_pose)
    while True:
        # t1 = time.time()
        while not rospy.is_shutdown():
...
def up(base_cyclic, z_tolerance=0.1, speed_mu_1=0.5):
    global wrist, finger_12
    # print('Move to: ', target_pose)
    while True:
        # t1 = time.time()
...
def reach():
    ...
    with utilities.DeviceConnection.createTcpConnection(args) as router:
        ...
        target_pose[2] = 0.036
        move_gripper(0.00)
        pick(base_cyclic, tolerance=0.008, speed_mu_1=0.6)
        move_gripper(0.50)

        target_pose[2] = 0.200
        up(base_cyclic, speed_mu_1=0.6)

        target_pose = chess_new_pose[target_id]
        place(base_cyclic, speed_mu_1=0.6)
        move_gripper(0.00)
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        if rospy.has_param('/chess_available'):
            chess_available = np.array(rospy.get_param("/chess_available"))
            target_id = np.min(chess_available)
            break
    reach()
    # args = utilities.parseConnectionArguments()
    # with utilities.DeviceConnection.createTcpConnection(args) as router:
