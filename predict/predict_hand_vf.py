...
class predict_2:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if rospy.has_param('/finger_0') and rospy.has_param('/chess_0'):
                finger_0 = np.array(rospy.get_param("/finger_0"))
                chess_0 = np.array(rospy.get_param("/chess_2"))
            print(np.sum((finger_0 - chess_0) ** 2))
...
class predict_hand_vf:
    ...
    def __init__(self):
        ...
        self.handtraj_sub = rospy.Subscriber('/hand_traj', JointTrajectoryPoint, self.handtraj_cb)
    ...
    def handtraj_cb(self, handtraj):
        ...
        # print(handtraj.positions)

        # Perform length check on message arrays
        if len(handtraj.positions) != len(self.wp) or len(handtraj.velocities) != len(self.v):
            print(
                'handtraj_cb: handtraj.positions and handtraj.velocities are not the same length as self.wp and self.v')
            return
        ...
        if np.abs(handtraj.velocities[0]) != 0 or np.abs(handtraj.velocities[1]) != 0 or np.abs(
                handtraj.velocities[2]) != 0:
            self.time_since_reset = rospy.get_time()
        ...
        if rospy.get_time() - self.time_since_reset > self.reset_time_threshold:
            ...
            u, v, w, behind = self.vf.updateVF(self.v[0], self.v[1], self.v[2], self.wp[0], self.wp[1], self.wp[2],
                                               alpha)  #
            ...
            for obj in self.object_vfs:
                ...
                updated_vf = dt.dynamicVF3D(self.trajloc, self.trajvel, self.trajtime, obj['h'][0], obj['h'][1],
                                            obj['h'][2], obj['vf'], self.beta)
            ...
            # else:
            #     print("None and the scores are above 0.5: ", self.score_list)
    ...
    def initialise_vectorfields(self, object_data):
        ...
        for obj_id in [0, 1, 2]:
            param_name = '/chess_' + str(obj_id)
            obj_p = None
            while not rospy.is_shutdown():
                if rospy.has_param(param_name):
                    obj_p = rospy.get_param(param_name)
                    break
            vf_obj = vf3D.VectorField3D(self.n[0], self.n[1], self.n[2], self.disc_param)
            self.object_vfs.append({'id': obj_id, 'h': obj_p, 'vf': vf_obj, 'score': 0})
